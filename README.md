...
Students create their own account from the sign-in page using their name, student ID, email, and password. Accounts are persisted in SQLite; passwords are salted and hashed by the backend and are never stored in the frontend.

The reset database includes sample users and a warden account solely for local development and the sample grievance data. New student grievances are associated with the authenticated user's database ID.
